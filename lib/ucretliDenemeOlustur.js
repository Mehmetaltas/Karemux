...
[{"soru":"...","secenekler":["A) ...","B) ...","C) ...","D) ..."],"dogruIndex":0,"altKonu":"...","aciklama":"..."}]`;
  ...
  const jsonParcasi = temiz.slice(baslangic, bitis + 1);
  let sorularHam;
  try {
    sorularHam = JSON.parse(jsonParcasi);
  } catch (parseHatasi) {
    console.error("GECICI DEBUG - JSON parse hatasi, tam parca (uzunluk " + jsonParcasi.length + "):", jsonParcasi);
    throw parseHatasi;
  }

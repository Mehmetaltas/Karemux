...
// Sayisal olmayan (olgusal/analiz/dilbilgisi kazanimli) dersler - bu derslerde
// yapay matematik/finansal hesap sorusu CIKMAMALI. Matematik ve Fen Bilimleri
// bu listede YOK (orada sayisal soru dogal ve bekleniyor).
const SAYISAL_OLMAYAN_DERSLER = ["Turkce", "Sosyal Bilgiler", "T.C. Inkilap Tarihi", "Din Kulturu", "Ingilizce"];

// Butce/enflasyon/yuzde/para gibi yapay matematik izleri - 18 Eylul'de
// GERCEK bir orenkte bulundu (Inkilap Tarihi dersinde butce hesabi sorusu).
const YAPAY_MATEMATIK_DESENI = /%\s?\d|\d\s?%|₺|\bTL\b|\bdolar\b|\blira\b|\bmilyar\b|\bmilyon\b|b[üu]tçe|enflasyon|\bvergi\b|maliyet|(gelir|toplam).{0,20}(ka[çc])/i;

// Katman 4 - Mufredat sinir kontrolu (18 Eylul, GENISLETILDI). Iki ayri kontrol:
// (1) Referans var mi - eskisi gibi. (2) YENI - sayisal olmayan derslerde
// SORU METNININ kendisinde (anlatim/derinlestir'deki disiplinlerarasi baglanti
// DEGIL, sadece degerlendirilecek SORULAR) yapay matematik/finansal hesap
// izi var mi - GERCEK bir kalite kusurunu (Inkilap Tarihi'nde butce sorusu)
// yakalamak icin eklendi.
export function mufredatSinirKontrolYap(ders, sorular = []) {
  const uyarilar = [];
  ...
  if (SAYISAL_OLMAYAN_DERSLER.includes(ders)) {
    sorular.forEach((s, i) => {
      const tamMetin = [s.soru || "", ...(s.secenekler || [])].join(" ");
      if (YAPAY_MATEMATIK_DESENI.test(tamMetin)) {
        uyarilar.push(`soru[${i}]: "${ders}" dersinin kazanim turuyle UYUMSUZ yapay matematik/finansal hesap iceriyor`);
      }
    });
  }
  return { gecti: uyarilar.length === 0, uyarilar };

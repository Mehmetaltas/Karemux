...
async function geminiCagir({ prompt, imageBase64, mediaType, maxTokens, jsonModu }) {
  ...
  const generationConfig = { maxOutputTokens: maxTokens || 1000, thinkingConfig: { thinkingBudget: 0 } };
  if (jsonModu) generationConfig.responseMimeType = "application/json";
...
async function openrouterCagir({ prompt, imageBase64, maxTokens, jsonModu }) {

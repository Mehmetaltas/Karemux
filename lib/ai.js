...
function aiKullanimLogla(saglayici, basarili, sureMs, jsonModu) {
  ...
  import("@/lib/db").then(({ sql }) => {
    sql`INSERT INTO ai_saglayici_log (saglayici, basarili, sure_ms, json_modu) VALUES (${saglayici}, ${basarili}, ${sureMs}, ${jsonModu})`.catch(() => {});
  }).catch(() => {});
}
...
export async function aiCagir(parametreler) {
  ...
  for (const ad of sira) {
    ...
    try {
      ...
      const jsonBasiMi = /^[{[]/.test(sonuc.trim());
      ...
      aiKullanimLogla(ad, true, Date.now() - baslangicZamani, !!parametreler.jsonModu);
      return sonuc;
    } catch (e) {
      aiKullanimLogla(ad, false, Date.now() - baslangicZamani, !!parametreler.jsonModu);

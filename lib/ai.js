...
export async function aiCagirDetay(parametreler, ozelSira = null, zamanBudcesiMs = 45000) {
  // 21 Eylul: GERCEK kanit (GitHub Actions loglari) - saglayicilar (Gemini kota/
  // Anthropic kredi) sorunluyken TEK BIR 4-saglayicili dongu bile Gemini'nin 429
  // yanitindaki gercek bekleme sureleri (15-50sn) yuzunden BASLI BASINA Vercel'in
  // 60sn siniri yaklasabiliyordu (kanit: odev_paketi/eksik_konu_paketi tam
  // 60.000sn'de kesildi, cagiran route JSON formatli hata bile veremedi). Zaman
  // butcesi asilinca KALAN saglayicilar denenmez, aggregate hata hemen firlatilir -
  // boylece cagiran route'un kendi hata isleyicisi calisma sansi bulur.
  const cagriBaslangici = Date.now();
  ...
  for (const ad of sira) {
    ...
    if (Date.now() - cagriBaslangici > zamanBudcesiMs) {
      hatalar.push(`${ad}: zaman butcesi asildi, denenmedi`);
      break;
    }
...
export async function aiCagir(parametreler, zamanBudcesiMs) {
  const { metin } = await aiCagirDetay(parametreler, null, zamanBudcesiMs);
  return metin;
}
...
SADECE JSON dondur: {"hataVarMi": true/false, "bulgular": ["varsa, kisa ve net hata aciklamasi"]}`;
    const { metin } = await aiCagirDetay({ prompt: p, maxTokens: 1000, jsonModu: true, tur: "capraz_dogrulama" }, alternatifSira, 12000); // kisa butce - bu opsiyonel/best-effort bir kontrol, ana uretimi geciktirmemeli
    // 24 Eylul: eski kirilgan parse (bugun defalarca bulunan AYNI hata sinifi)
    // guvenli jsonAyikla()'ya gecirildi.
    return jsonAyikla(metin);
  } catch (e) {

...
export async function POST(req) {
  try {
    ...
    if (tanim.ciktiTipi === "acik_uclu") {
      const p = `Sen bir LGS/ortaokul ogretmenisin. "${ders}" dersinden "${konu}" konusuyla ilgili ${sinif}. sinif seviyesinde bir "${tanim.baslik}" hazirla: ${tanim.aciklama}. ${BAGLAM_TEMELLI_SORU_TALIMATI}${kaliteReferansi ? " Kalite referansi: " + kaliteReferansi : ""} ONEMLI: Bu sorular ACIK UCLU olmali - coktan secmeli SIK (A/B/C/D) OLMAMALI, ogrenci kendi cozumunu yazmali. Her soru icin "cozum" alaninda, ogrencinin kontrol edebilecegi ADIM ADIM, DETAYLI bir cozum ver (sadece sonuc degil, tum adimlari goster) - cozumu, sicak bir ogretmenin evde tek basina calisan ogrenciye yaninda oturup anlatiyormus gibi yaz, kisa cumleler ve "bak", "simdi" gibi dogal ifadeler kullan, resmi/ansiklopedik dilden kacin. SADECE JSON dondur, markdown kullanma. Tum metinler SADECE Turkce olmali:

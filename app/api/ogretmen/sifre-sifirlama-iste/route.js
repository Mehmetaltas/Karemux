...
// Ogretmen sifresini unutunca kullanilir - ayni "sifre-belirle" akisini
// (yeni token uretip e-posta gonderme) tekrar tetikler.
export async function POST(req) {
  try {
    const ip = istekIpAdresi(req);
    const kontrol = await denemeSiniriKontrolEt(ip, "ogretmen_sifre_sifirlama", 3, 15);
    ...
    const ogretmen = await sql`SELECT id, ad FROM ogretmenler WHERE eposta = ${eposta.trim().toLowerCase()} AND aktif = true`;
    await denemeKaydet(ip, "ogretmen_sifre_sifirlama", true);

    // Guvenlik: hesap yoksa bile ayni "gonderildi" mesajini donuyoruz -
    // boylece kayitli eposta listesi disaridan taranamiyor.
    if (ogretmen.length === 0) return Response.json({ ok: true });

    const token = veliOnayTokenUret();
    await sql`UPDATE ogretmenler SET sifre_belirleme_token = ${token}, sifre_belirleme_son_tarih = now() + interval '48 hours' WHERE id = ${ogretmen[0].id}`;
    ...
    } catch (e) { console.error("Sifre sifirlama e-postasi gonderilemedi:", e); }

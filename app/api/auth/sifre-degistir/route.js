...
// Oturum acikken kendi sifresini degistirme (8-9 Eylul) - kullanicilar
// tablosu OGRENCI/VELI/KURUM YONETICISI'nin ucunu de kapsar.
export async function POST(req) {
  try {
    const ip = istekIpAdresi(req);
    const kontrol = await denemeSiniriKontrolEt(ip, "kullanici_sifre_degistir", 5, 15);
    ...
    const satir = await sql`SELECT sifre_hash FROM kullanicilar WHERE id = ${kullaniciId}`;
    if (satir.length === 0) return Response.json({ error: "Hesap bulunamadi" }, { status: 404 });
    if (satir[0].sifre_hash === "anon") {
      return Response.json({ error: "Bu hesapta henuz bir sifre yok (misafir/cihaz hesabi). Once eposta ile kayit olmalisin." }, { status: 400 });
    }

    const dogru = await sifreDogrula(eskiSifre, satir[0].sifre_hash);
    if (!dogru) {
      await denemeKaydet(ip, "kullanici_sifre_degistir", false);
      return Response.json({ error: "Eski sifre hatali" }, { status: 401 });
    }
    await denemeKaydet(ip, "kullanici_sifre_degistir", true);

    const yeniHash = await sifreHashle(yeniSifre);
    await sql`UPDATE kullanicilar SET sifre_hash = ${yeniHash} WHERE id = ${kullaniciId}`;

...
export default function KurumGiris() {
  ...
  useEffect(() => {
    try {
      const kayitli = localStorage.getItem("karemux_hatirla_kurum");
      if (kayitli) {
  ...
  async function girisYap() {
    ...
    try {
      ...
      try {
        if (beniHatirla) {
          localStorage.setItem("karemux_hatirla_kurum", JSON.stringify({ eposta, sifre }));
        } else {
          localStorage.removeItem("karemux_hatirla_kurum");
        }
      } catch (e) {}
